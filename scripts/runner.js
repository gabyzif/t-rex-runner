...
(function () {
  ...
  function Runner(outerContainerId, opt_config) {
    ...
    this.obstaclesClearedSinceLastSurprise = 0;
  ...
  Runner.surprises = [
    { type: 'video', id: 'surprise-01', src: 'videos/surprise-01.mp4' },
    { type: 'video', id: 'surprise-02', src: 'videos/surprise-02.mp4' },
    { type: 'video', id: 'surprise-03', src: 'videos/surprise-03.mp4' },
    { type: 'video', id: 'surprise-04', src: 'videos/surprise-04.mp4' },
    { type: 'video', id: 'surprise-05', src: 'videos/surprise-05.mp4' },
    { type: 'video', id: 'surprise-06', src: 'videos/surprise-06.mp4' },
    { type: 'mosaic', id: 'videos/final' }
  ];

  Runner.finalPhotos = Array.from({ length: 120 }, (_, i) => `images/final/photo-${i + 1}.jpg`);
  ...
  Runner.prototype = {
    ...
    hasMoreSurprises: function () {
      return this.surpriseIndex < Runner.surprises.length - 1;
    },
    triggerNextSurprise: function () {
      if (this.isShowingSurprise) return;
      if (this.surpriseIndex >= Runner.surprises.length) return;

      const surprise = Runner.surprises[this.surpriseIndex];

      // 🚫 el final NO entra por acá
      if (surprise.type === 'mosaic') return;

      this.surpriseIndex++;
      this.obstaclesClearedSinceLastSurprise = 0;
      this.pauseForSurprise(surprise);
    },

    pauseForSurprise: function (surprise) {
      this.isShowingSurprise = true; // 🔒

      this.stop();

      switch (surprise.type) {
        case 'video':
          this.showVideoSurprise(surprise);
          break;
        case 'mosaic':
          this.showFinalMosaic();
          break;
      }
    },
    /**
    ...
    playIntro: function () {
      if (!this.started && !this.crashed) {
        ...
        // arrancar directo, sin CSS dinámico
        this.startGame();
     ...
     */
    update: function () {
      console.log('update frame', this.crashed);
    ...
    // when the user hits an obstacle
    onHit: function () {
      if (this.isShowingSurprise) return; // 🧠 guard extra

      this.lives--;
      this.updateLivesUI(); // 👈 nuevo

      if (this.horizon.obstacles[0]) {
        this.horizon.obstacles[0].hasCollided = true;
      }

      if (!this.hasMoreSurprises()) {
        this.gameOver();
        return;
      }

      this.triggerNextSurprise();
    },

    onObstacleCleared: function () {
      this.obstaclesClearedSinceLastSurprise++;
      if (this.obstaclesClearedSinceLastSurprise >= 3) {
        this.obstaclesClearedSinceLastSurprise = 0;
        this.triggerNextSurprise();
      }
    },

    recoverFromHit: function () {
      // limpiar obstáculos actuales
      this.horizon.reset();

      // devolver el dino al suelo
      this.tRex.reset();

      // resetear timers para evitar doble hit inmediato
      this.runningTime = 0;
    ...
    gameOver: function () {
      ...
      new GameOverPanel(this.canvas, this.images.TEXT_SPRITE, this.images.RESTART, this.dimensions);
    ...
    showFinalMosaic: function () {
      const layer = document.getElementById('surprise-layer');
    ...
    startPhotoShower: function () {
      const container = document.getElementById('photo-shower');

      Runner.finalPhotos.forEach((src, index) => {
    ...
    // pauseForSurprise: function () {
    //   this.stop(); // pausa el juego
    //   this.showVideoSurprise();
    // },
    showVideoSurprise: function (surprise) {
      const layer = document.getElementById('surprise-layer');
      const video = document.getElementById('surprise-video');
      const playBtn = layer.querySelector('.surprise__close');
      ...
      playBtn.onclick = () => {
        video.pause();
        video.currentTime = 0;
        video.removeAttribute('src'); // 🔥 evita bugs raros
        video.load();

        layer.classList.remove('is-active');
        layer.setAttribute('aria-hidden', 'true');

        this.resumeAfterSurprise();
      };
    },
    resumeAfterSurprise: function () {
      this.isShowingSurprise = false; // 🔓
      this.recoverFromHit();
      this.time = performance.now();
      this.updateLivesUI(); // 👈 sync visual
  ...
  Horizon.prototype = {
    ...
    updateObstacles: function (deltaTime, currentSpeed) {
      ...
      for (var i = 0; i < this.obstacles.length; i++) {
        ...
        if (obstacle.remove && !obstacle.hasCollided) {
          Runner.instance_.onObstacleCleared();
        }
      }
